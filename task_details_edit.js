...
firebase.database().ref('task-details-form/' + taskKey).once('value').then((snapshot) => {
    ...
    document.getElementById('story-point-of-task').value = taskDetails.taskStoryPoint || '';
...
document.getElementById('done-button').addEventListener('click', function(event) {
    ...
    let taskStoryPoint = document.getElementById("story-point-of-task").value.trim();
    ...
    if (!formValid) {
        ...
        firebase.database().ref('task-details-form/' + taskKey).update(updatedTask, function(error) {
            if (error) {
                ...
                window.location.href = "index.html";  // Redirect to product backlog page after saving
            }
        });
    }
});